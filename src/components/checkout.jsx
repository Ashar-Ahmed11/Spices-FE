...
export default function Checkout() {
    ...
    const { basicInfo, openRef, city, setCity, history, country, cart, fetchCart, checkouter, setcheckouter, setMainLoader, setImgIsLoaded, sendWhatsappMessage, checkoutLoader, totalCal } = context
    useEffect(() => {
    ...
    const declareMessage = (e) => {
        e.preventDefault()
        sendWhatsappMessage(form.firstname + " " + form.lastname, form.email, form.phone, products, form.address, city, theSubtotal, totalCal)
    }

    // const total = cart.line_items.subtotal.formatted_with_code
    // console.log(parseInt(total))

    const total = city == "Karachi" ? totalCal + basicInfo?.karachiDelivery || 0 : totalCal + basicInfo?.otherDelivery || 0
                    ...
                    <div class=" px-4">
                        ...
                        <div className='mt-4' style={{ color: color }}>
                            ...
                            <div style={{ borderBottom: '1px solid #dadada' }} className="d-flex justify-content-between">
                                ...
                                <p>PKR {city=="Karachi"?basicInfo?.karachiDelivery:basicInfo?.otherDelivery}</p> 
                    ...
                    <form onSubmit={(e) => declareMessage(e)}>
                        ...
                        <div class="mb-3">
                            ...
                            <select required placeholder='Country/Region' style={{ backgroundColor: '#ffffff', borderColor: "#dedede", color: 'black' }} class="form-select" aria-label="Default select example">
                                <option selected>{country == "Pakistan" && "Pakistan"}</option>
                            ...
                            <div className="d-flex justify-content-between my-4">
                                <input name="no-autofill" autoComplete="new-password" onClick={() => openRef.current.click()} required value={city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder='City' style={{ backgroundColor: '#ffffff', borderColor: "#dedede", color: 'black' }} type="text" class="form-control mx-1" />
